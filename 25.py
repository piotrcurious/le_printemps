...
def get_local_state(grid, r, c, size):
...
def evaluate_basis_local(grid_chunk, r_start, c_start, basis, local_symbols, p, size):
    rows, cols = grid_chunk.shape
    results = np.zeros_like(grid_chunk, dtype=np.uint8)
    for r_local in range(rows):
        for c_local in range(cols):
            r_global = r_start + r_local
            c_global = c_start + c_local
            local_state_values = get_local_state(grid_chunk, r_local, c_local, size)
            symbol_values = dict(zip(local_symbols, local_state_values))
            basis_evaluations = [poly.eval(symbol_values) % p for poly in basis]
            if any(eval_val != 0 for eval_val in basis_evaluations):
                results[r_local, c_local] = 1
    return results, r_start, c_start

# --- Game of Life Guided by Gröbner Basis (Parallel and GPU Attempt) ---
class GameOfLifeGroebnerAdvanced:
    ...
    def update(self):
        ...
        if self.basis:
            chunk_size = 10 # Adjust based on performance
            chunks = []
            for r in range(0, rows, chunk_size):
                for c in range(0, cols, chunk_size):
                    ...
                    chunks.append((chunk, r, c))

            if self.pool:
                results = self.pool.starmap(evaluate_basis_local, [(chunk, r, c, self.basis, self.local_symbols, self.p, self.local_size) for chunk, r, c in chunks])
                basis_violation = np.zeros_like(state, dtype=np.uint8)
                for res_chunk, r_start, c_start in results:
                    basis_violation[r_start:r_start + res_chunk.shape[0], c_start:c_start + res_chunk.shape[1]] = res_chunk
            ...
            else: # Fallback if no multiprocessing
                basis_violation = np.zeros_like(state, dtype=np.uint8)
                for r in range(rows):
                    for c in range(cols):
...
def draw_grid(screen, game_of_life):
    ...
    pygame.display.set_caption(f"AI Spring: Stage {poem_stage}, Line: '{line}', Pattern: {game_of_life.target_pattern}, Basis Size: {len(game_of_life.basis) if game_of_life.basis else 0}, Parallel: {game_of_life.pool is not None}, GPU: {game_of_life.use_gpu}")
    ...
    for r in range(rows):
        for c in range(cols):
            ...
            if game_of_life.basis:
                local_state_values = game_of_life.get_local_state(display_state, r, c, game_of_life.local_size)
...
def main():
    ...
    game = GameOfLifeGroebnerAdvanced(GRID_SIZE, PRIME_FIELD, LOCAL_SIZE)
