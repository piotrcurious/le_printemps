...
PRIME_FIELD = 5  # Experiment with different prime numbers
    ...
    def update(self, fractal_equation="circle", fractal_params=None):
        ...
        for row in range(self.grid_size):
            for col in range(self.grid_size):
                ...
                # Define the update rule as a polynomial over Z_p, influenced by fractal_value
                # next_state = (A * current_state^2 + B * neighbor_sum + C * fractal_value^2 + D) mod p

                # Coefficients influenced by fractal value (more complex)
                influence = fractal_value * 5  # Scale the fractal value

                A = Zp(int(abs(influence) * 0.5) % self.p, self.p)
                B = Zp((int(influence * 0.8) + 1) % self.p, self.p)
                C = Zp(int(abs(influence) * 0.3) % self.p, self.p)
                D = Zp(int(influence * self.p) % self.p, self.p)

                neighbor_sum_zp = Zp(neighbors, self.p)
                fractal_value_zp = Zp(int(influence * 100), self.p) # Scale to have effect

                term1 = A * (current_state ** 2)
                term2 = B * neighbor_sum_zp
                term3 = C * (fractal_value_zp ** 2)
                next_state_zp = term1 + term2 + term3 + D
...
def main():
    ...
    initial_seed_count = 4000  # Increase seeding further
